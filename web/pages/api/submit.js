...
async function processVideoPreview({ url, email, videoId, clientIP, userAgent }) {
  try {
    ...
    // 发送预览邮件
    await sendPreviewEmail(email, mockPreviewResult, url, videoId);
    ...
    logSecurityEvent({
      ...
      details: {
        email,
        videoId,
        success: mockPreviewResult.success,
        cost: mockPreviewResult.cost_analysis?.total_cost_usd
      },
      severity: 'info'
    });
...
// 发送预览邮件
async function sendPreviewEmail(email, previewResult, url, videoId) {
  // 这里应该集成真实的邮件发送服务
  console.log(`发送预览邮件到: ${email}`);
  console.log(`视频链接: ${url}`);
  console.log(`视频ID: ${videoId}`);
  console.log(`预览结果:`, previewResult);
  
  // TODO: 集成真实的SMTP邮件发送
  // 可以使用 nodemailer 等库发送邮件
